  ...
  return (
    <Document>
      <Page size="A4" style={styles.titlePage}>
        {data.logo_url && (
          <Image src={data.logo_url} style={{ width: 150, marginBottom: 40 }} />
        )}
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.scorecard} break={false}>
          <View style={[styles.scoreItem, { backgroundColor: "#dc2626" }]}>
            <Text style={styles.scoreVal}>{counts.critical || 0}</Text>
            <Text style={styles.scoreLabel}>Critical</Text>
          </View>
          <View style={[styles.scoreItem, { backgroundColor: "#ea580c" }]}>
            <Text style={styles.scoreVal}>{counts.high || 0}</Text>
            <Text style={styles.scoreLabel}>High</Text>
          </View>
          <View style={[styles.scoreItem, { backgroundColor: "#ca8a04" }]}>
            <Text style={styles.scoreVal}>{counts.medium || 0}</Text>
            <Text style={styles.scoreLabel}>Medium</Text>
          </View>
          <View style={[styles.scoreItem, { backgroundColor: "#16a34a" }]}>
            <Text style={styles.scoreVal}>{counts.low || 0}</Text>
            <Text style={styles.scoreLabel}>Low</Text>
          </View>
          <View style={[styles.scoreItem, { backgroundColor: "#2563eb" }]}>
            <Text style={styles.scoreVal}>{counts.informational || 0}</Text>
            <Text style={styles.scoreLabel}>Info</Text>
          </View>
        </View>

        <Text style={styles.sectionTitle}>1. Executive Summary</Text>
        <Text style={{ marginBottom: 20 }}>
          {data.executive_summary || "No summary."}
        </Text>

        <Text style={styles.sectionTitle}>2. Scope</Text>
        <Text style={{ marginBottom: 20 }}>
          {(data.targets || [])
            .map((t: any) => `${t.host} (${t.ip})`)
            .join(", ") || "No targets."}
        </Text>
        ...
        {(data.vulnerabilities || [])
          ...
          .map((v: any) => (
            <View key={v.id} style={styles.finding} wrap={true}>
              ...
              <View wrap={true}>
                {(Array.isArray(v.screenshot_url)
                  ? v.screenshot_url
                  : [v.screenshot_url]
                )
                  .filter(Boolean)

...
import { 
  Save, Trash2, Plus, Menu, ShieldAlert, FileSearch, CheckCircle2, RefreshCw, X, Trash
} from "lucide-react"
...
const SortableFindingItem = ({ vuln, selected, onClick }: { vuln: any, selected: boolean, onClick: () => void }) => {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: vuln.id });
  const style = { transform: CSS.Transform.toString(transform), transition };
  return (
    <div ref={setNodeRef} style={style} className={cn("flex items-center gap-2 border-b", selected ? "bg-primary/5" : "hover:bg-muted/50")}>
      <div {...attributes} {...listeners} className="cursor-grab p-4 text-muted-foreground"><Menu className="size-4" /></div>
      <button onClick={onClick} className="flex-1 flex items-center justify-between p-4 pl-0 text-left">
        <div className="space-y-1 overflow-hidden">
          <p className="text-sm font-medium leading-none truncate">{vuln.synopsis}</p>
          <p className="text-[10px] text-muted-foreground font-mono">{vuln.severity} ({vuln.cvss_score?.toFixed(1) || 0})</p>
        </div>
      </button>
    </div>
  )
...
export function ReportEditor({ initialData }: { initialData: any }) {
  ...
  const [selectedVulnId, setSelectedVulnId] = useState<string | null>(
    initialData.vulnerabilities?.length > 0 ? initialData.vulnerabilities[0].id : null
  )
  ...
  const selectedVuln = report.vulnerabilities.find((v: any) => v.id === selectedVulnId)

  const debouncedSave = useCallback(
    debounce((updatedReport: any) => {
      // Only persist to DB if report is already published
      if (updatedReport.status !== 'published') return;
      ...
      supabase
        ...
        .update({
          ...
          client_id: updatedReport.client_id,
          executive_summary: updatedReport.executive_summary || "",
          targets: updatedReport.targets || [],
          confidentiality_statement: updatedReport.confidentiality_statement || "",
        })
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = report.vulnerabilities.findIndex((v: any) => v.id === active.id)
      const newIndex = report.vulnerabilities.findIndex((v: any) => v.id === over.id)
      const newVulns = arrayMove(report.vulnerabilities, oldIndex, newIndex)
      handleUpdateReport("vulnerabilities", newVulns.map((v: any, i: number) => ({ ...v, order_index: i })))
    }
  }
  ...
  const handleAddTarget = () => {
    handleUpdateReport("targets", [...(report.targets || []), { id: generateId(), host: "", ip: "", port: "" }])
  }

  const handleUpdateTarget = (id: string, field: string, value: string) => {
    const newTargets = report.targets.map((t: any) => t.id === id ? { ...t, [field]: value } : t)
    handleUpdateReport("targets", newTargets)
  }

  const handleHostChange = (id: string, host: string) => {
    const newTargets = report.targets.map((t: any) => t.id === id ? { ...t, host } : t)
    setReport((prev: any) => ({ ...prev, targets: newTargets }))
    debouncedLookup(id, host)
    saveDraft({ ...report, targets: newTargets })
  }
  ...
  const handleDeleteTarget = (id: string) => {
    handleUpdateReport("targets", report.targets.filter((t: any) => t.id !== id))
  }
  ...
  const handlePublish = async () => {
    ...
    // 1. Process images
    const updatedVulnerabilities = await Promise.all(report.vulnerabilities.map(async (v: any) => {
      const urls = Array.isArray(v.screenshot_url) ? v.screenshot_url : [v.screenshot_url].filter(Boolean)
      const updatedUrls = await Promise.all(urls.map(async (url: string) => {
        if (typeof url === 'string' && url.startsWith('blob:')) {
          const response = await fetch(url)
          const blob = await response.blob()
          const file = new File([blob], `evidence-${v.id}-${generateId()}.webp`, { type: 'image/webp' })
          const formData = new FormData()
          formData.append("file", file)
          formData.append("path", `findings/${report.id}/${v.id}/${file.name}`)
    ...
    const { error: vulnError } = await supabase.from("vulnerabilities").upsert(
      updatedVulnerabilities.map((v: any) => ({ ...v, report_id: report.id }))
    )
  ...
  const handleDelete = async () => {
    const { error } = await supabase.from("reports").update({ deleted_at: new Date().toISOString() }).eq("id", report.id)
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-3">
             <h1 className="text-2xl font-bold tracking-tight truncate">{report.title || "Untitled Report"}</h1>
             <StatusBadge variant={(report.status || 'draft') === 'published' ? 'published' : 'draft'}>
               {(report.status || 'draft').toUpperCase()}
             </StatusBadge>
          </div>
          <p className="text-muted-foreground text-sm mt-1">Created: {new Date(report.created_at).toLocaleDateString()}</p>
        </div>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>This will move the report to trash.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>Save this report? You can edit it later.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
        ...
        <TabsContent value="overview" className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={report.title} onChange={(e) => handleUpdateReport("title", e.target.value)} placeholder="e.g. Q1 Assessment" />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <span className={cn("text-xs", (report.executive_summary?.length || 0) > 10000 ? "text-destructive" : "text-muted-foreground")}>
                ...
                <Textarea 
                  ...
                  onChange={(e) => handleUpdateReport("executive_summary", e.target.value)} 
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={handleAddTarget}><Plus className="size-4 mr-2" /> Add Target</Button>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {(report.targets || []).map((t: any) => (
                    <TableRow key={t.id}>
                      <TableCell>
                        <Input required value={t.host || ""} onChange={(e) => handleHostChange(t.id, e.target.value)} placeholder="e.g., https://api.kira.com" />
                      </TableCell>
                      <TableCell>
                        <Input required value={t.ip || ""} onChange={(e) => handleUpdateTarget(t.id, "ip", e.target.value)} placeholder="e.g., 10.0.0.1" />
                      </TableCell>
                      <TableCell>
                        <Input required value={t.port || ""} onChange={(e) => handleUpdateTarget(t.id, "port", e.target.value)} placeholder="e.g., 443" />
                      </TableCell>
                      <TableCell><Button variant="ghost" size="icon" onClick={() => handleDeleteTarget(t.id)}><X className="size-4 text-destructive" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
            <Card className="h-fit max-h-[calc(100vh-250px)] overflow-hidden flex flex-col">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b">
                ...
                <Button variant="ghost" size="icon" onClick={handleAddVulnerability}>
                  <Plus className="size-4" />
                </Button>
              </CardHeader>
              <CardContent className="p-0 overflow-y-auto">
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                  <SortableContext items={report.vulnerabilities.map((v: any) => v.id)} strategy={verticalListSortingStrategy}>
                    <div className="flex flex-col">
                      {(report.vulnerabilities || []).length === 0 ? (
                        ...
                        report.vulnerabilities.map((vuln: any) => (
                          <SortableFindingItem key={vuln.id} vuln={vuln} selected={selectedVulnId === vuln.id} onClick={() => setSelectedVulnId(vuln.id)} />
                        ))
                      )}
                    </div>
                  </SortableContext>

...
export function ProfileSettings({ user, profile }: { user: any, profile: any }) {
  ...
  const [pendingFiles, setPendingFiles] = useState<{avatar?: File, logo?: File}>({})
  ...
  const handleImageSelection = async (e: React.ChangeEvent<HTMLInputElement>, type: 'avatar' | 'logo') => {
    ...
    if (type === 'logo') {
      ...
      try {
        await new Promise((resolve, reject) => {
          img.onload = () => {
            if (img.width < 200 || img.height < 200) reject("Logo min 200x200px")
            if (img.width > 2000 || img.height > 2000) reject("Logo max 2000x2000px")
            resolve(true)
          }
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              <button onClick={() => document.getElementById('avatar-upload')?.click()} className="absolute -bottom-1 -right-1 p-1.5 rounded-full bg-primary text-primary-foreground shadow-lg transition-transform hover:scale-110">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            <div className="size-32 rounded-lg border border-dashed flex items-center justify-center bg-muted/20 overflow-hidden">
              {previews.logo ? <img src={previews.logo} alt="Logo" className="size-full object-contain p-4" /> : <ImageIcon className="size-10 text-muted-foreground/30" />}
            </div>
            <div className="space-y-2 flex-1">
              <Button variant="outline" size="sm" onClick={() => document.getElementById('logo-upload')?.click()}>
                Select Logo
              </Button>
              <input id="logo-upload" type="file" accept="image/png, image/jpeg, image/webp" className="hidden" onChange={(e) => handleImageSelection(e, 'logo')} />
